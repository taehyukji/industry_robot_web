...
const scoreColors = ["#0f8b8d", "#3a9b5f", "#d79922", "#52616d"];
const scoreLabels = ["위치 점수", "효율 점수", "경제성 점수", "종합 점수"];
...
function calculateScores(adjusted, metrics, refAdjusted, refMetrics) {
  const location = 100 * (
    0.45 * (factory.locationScore / factory.locationScore)
    + 0.30 * (factory.transportCost / factory.transportCost)
    + 0.25 * (factory.supplierDelayMean / factory.supplierDelayMean)
  );
  ...
  const total = weightEfficiency * efficiency + weightEconomy * economy;
...
function renderChart(scores) {
  const values = [scores.location, scores.efficiency, scores.economy, scores.total];
  const maxValue = Math.max(120, ...values) * 1.08;
