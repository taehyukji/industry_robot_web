...
const fieldLimits = {
  ...
  payload: { min: 0.5, max: 30, fallback: 10 },
...
function runSimulation(adjusted, seed = 1) {
  ...
  for (let day = 0; day < config.days; day += 1) {
    ...
    const capacity = Math.round(adjusted.processRate * actualUptime * delayFactor);
    const requiredProduction = Math.ceil(demand / Math.max(1 - actualDefectRate, 0.01));
    const produced = Math.min(capacity, requiredProduction);
